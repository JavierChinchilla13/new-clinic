...
const ContactFormModal = ({ onClose }) => {
  ...
  const handleSubmitWhatsApp = (e) => {
    ...
    const message = `Hola, soy ${encodeURIComponent(formData.name)}%0A${encodeURIComponent(formData.details)}%0A%0A`;
    const emailText = `Mi email es ${encodeURIComponent(formData.email)}`;
    const whatsappUrl = `https://api.whatsapp.com/send?phone=50688829436&text=${message}${emailText}`;
  ...
  const products = [
    {
      ...
      type: "Facial",
      imageUrl:
        "https://via.placeholder.com/400?text=Exfoliaci%C3%B3n+Corporal",
    },
    {
      title: "Humectante",
